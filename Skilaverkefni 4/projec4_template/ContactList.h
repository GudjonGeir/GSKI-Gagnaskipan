...
class ContactList
{
	...
	bool add(string key, Contact value, NodePtr node);
	bool remove(string key, NodePtr node);
	bool contains(string key, NodePtr node);
        Contact get(string key, NodePtr node);
        void get_contacts(vector<Contact>& contacts, NodePtr node);
	vector<StringContactPair> to_vector(NodePtr node);
	void get_contacts_by_prefix(string prefix, vector<Contact>& contacts, NodePtr node);
};
